...
class Caffe_model_interf:
    def __init__(self, model_file, image_size=(96, 112)):
        import caffe

        caffe.set_mode_gpu()
        caffe.set_device(3)
        weight_file = model_file.replace(".prototxt", ".caffemodel")
...
class RKNN_model_interf:
    ...
    def __call__(self, imgs):
        ...
        for i, img in enumerate(imgs):
            ...
            output = self.rknn_lite.inference(inputs=[img])
            outputs[i] = output[0].flatten()
...
def get_embeddings(model_interf, img_names, landmarks, batch_size=64, flip=True):
    ...
    for batch_id in tqdm(
        ...
        ndimages = [
            face_align_landmark(
                cv2.imread(img), landmark, image_size=(96, 112)
            )  # model input size
...
class IJB_test:
    def __init__(
        ...
        if model_file != None:
            ...
            elif model_file.endswith(".rknn"):
                interf_func = RKNN_model_interf(model_file)
            elif model_file.endswith("prototxt"):
                interf_func = Caffe_model_interf(model_file)
            else:
            ...
            if "embs" in aa and "embs_f" in aa:
                ...
                print(
                    f"embs shape: {self.embs.shape}, embs_f shape: {self.embs_f.shape}"
                )
            else:
    ...
    def run_model_test_single(
        self, use_flip_test=True, use_norm_score=True, use_detector_score=True
    ):
    ...
    def run_model_test_1N(self, npoints=100):
        ...
        g2_templates_feature, g2_unique_templates, g2_unique_ids = (
            image2template_feature(
...
def parse_arguments(argv):
    ...
    if args.plot_only != None and len(args.plot_only) != 0:
        ...
        if (
            ...
            or args.model_file.endswith(".rknn")
            or args.model_file.endswith(".prototxt")
        ):
